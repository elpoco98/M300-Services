...
function fuCHECKPACKAGES {
  ...
  # Make sure dependencies for apt are installed
  myCURL=$(which curl)
  myWGET=$(which wget)
  mySUDO=$(which sudo)
  if [ "$myCURL" == "" ] || [ "$myWGET" == "" ] || [ "$mySUDO" == "" ]
    then
      echo "### Installing deps for apt"
      apt-get -y update
      apt-get -y install curl wget sudo
  fi
  echo "### Installing apt"
  /bin/bash -c "$(curl -sL https://raw.githubusercontent.com/ilikenwf/apt/master/quick-install.sh)"
...
#  fi
  ...
  apt -y dist-upgrade -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" --force-yes
  echo
  echo "### Installing T-Pot dependencies."
  echo
  apt -y install $myINSTALLPACKAGES
  # Remove exim4
  echo "### Removing and holding back problematic packages ..."
  apt -y purge exim4-base mailutils pcp cockpit-pcp
  apt -y autoremove
